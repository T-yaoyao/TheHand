 ...
 */
export async function extractFileInterfaces(sandboxPath, plan) {
    const interfaces = [];
 ...
 * 从文件内容中提取接口信息（纯正则）
 */
function extractInterfaceFromContent(filePath, content) {
 ...
 */
function resolveImportToFilePath(importPath, fromFile, planFilePaths) {
    ...
    for (const candidate of candidates) {
        if (planFilePaths.has(candidate))
            return candidate;
    }
    return null;
}
 ...
 */
function buildDependencyGraph(fileInterfaces, planFilePaths) {
    const graph = new Map();
    for (const fi of fileInterfaces) {
        if (!graph.has(fi.path))
            graph.set(fi.path, new Set());

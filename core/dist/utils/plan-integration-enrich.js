 ...
 */
export function enrichPlanWithIntegrationEntryFiles(plan, sandboxPath, projectContext, includeRouteEntryContext) {
    if (plan.length === 0)
        return plan;
    ...
    for (const f of plan) {
        ...
        if (f.priority > maxPri)
            maxPri = f.priority;
    }
    if (!includeRouteEntryContext) {
        return [...byPath.values()].sort((a, b) => a.priority - b.priority);
    }
    const addIfMissing = (rel, changeDescription, priority) => {
        const k = norm(rel);
        if (!k || byPath.has(k))
            return;
        if (!existsSync(resolve(sandboxPath, k)))
            return;
    ...
    if (entry) {
        addIfMissing(entry, '【TheHand 自动补充】与子路由/Tab/新页面相关：须在本文件注册或调整嵌套 <Route>（path 与布局内 NavItem/NavLink 一致），并保持根路由约定；相对 import 须与仓库实际路径一致（勿臆造如 ./components/App）。', maxPri + 5);
    }
    let extraPri = maxPri + 10;
    let readContextAdded = 0;
    for (const c of projectContext.thehand?.readContextCandidates ?? []) {
        if (readContextAdded >= 3)
            break;
        const k = norm(c);
        if (!k || byPath.has(k))
            continue;
        ...
        byPath.set(k, {
            path: k,
            changeDescription: '【TheHand 自动补充】项目配置的关键路由/上下文文件；若本次改动涉及路由表或根挂载，请读取并与现有结构对齐。',
            priority: extraPri++,
        });
        readContextAdded++;
...
//# sourceMappingURL=plan-integration-enrich.js.map
...
const SRC_EXT_ORDER = ['.tsx', '.ts', '.jsx', '.js', '.vue', '.mjs', '.cjs'];
/**
 ...
 */
export function skipOrphanImportIntegrationCheck(planPath, guard) {
    const p = planPath.replace(/\\/g, '/');
    for (const g of guard?.entrySkipGlobs ?? []) {
        if (g && matchPathGlob(p, g))
            return true;
    }
    if (/(^|\/)main\.(jsx?|tsx?)$/i.test(p))
        return true;
    if (/(^|\/)bootstrap\.(jsx?|tsx?|js|ts)$/i.test(p))
        return true;
    return false;
}
/** 归一化到「逻辑模块」键：index 桶文件、Foo/Foo.jsx 与目录名对齐 */
export function canonicalModuleKey(rel) {
...
function firstKnownModuleForResolved(resolved, known) {
...
function resolveSpecifierToModule(fromFilePosix, spec, known) {
    if (!spec.startsWith('.'))
        return null;
 ...
 */
export async function relativeSpecifierResolvesInSandbox(sandboxPath, fromFilePosix, specifier, knownSourcePaths) {
    if (!specifier.startsWith('.'))
        return true;
    const from = fromFilePosix.replace(/\\/g, '/');
    if (resolveSpecifierToModule(from, specifier, knownSourcePaths) !== null)
        return true;
...
export function extractRelativeImportSpecifiers(content) {
    const out = [];
    for (const rawLine of content.split('\n')) {
        ...
        if (!isImport && !isRequire && !isDynImport && !isReexport)
            continue;
        const re = /['"](\.[^'"]+)['"]/g;
        let m;
 ...
 */
export function sourceFileImportsTargetModule(content, fromPlanPath, targetPlanPath, knownFiles) {
    ...
    for (const spec of extractRelativeImportSpecifiers(content)) {
        ...
        if (hit && canonicalModuleKey(hit) === targetCanon)
            return true;
    }
    return false;
}
 ...
 */
export function sandboxIndexImportsComponent(index, componentPlanPath, _baseName) {
    ...
    for (const [rel, content] of index) {
        ...
        if (rel === norm)
            continue;
        if (sourceFileImportsTargetModule(content, rel, norm, known))
            return true;
    }
    return false;
}
async function collectSourceFilesRecursive(dir, acc) {
    let entries;
    try {
    ...
    for (const e of entries) {
        if (e.name === 'node_modules' || e.name === 'dist' || e.name === '.git' || e.name === 'coverage')
            continue;
...
function candidateSourceRoots(sandboxPath, structure) {
    const roots = [];
    const pushIf = (rel) => {
        if (!rel)
            return;
 ...
 */
export async function loadSandboxSourceContents(sandboxPath, structure) {
    const out = new Map();
    const paths = [];
...
//# sourceMappingURL=component-sandbox-import.js.map
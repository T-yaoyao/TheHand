...
const FIELDS_REQUIRED = ['add_field', 'delete_field'];
...
// 合法的 scope 枚举
const VALID_SCOPES = ['frontend', 'backend', 'fullstack'];
/**
 * 校验结构化需求的完整性
 * 在澄清完成后、进入 Plan 阶段之前调用
 */
export function validateRequirement(req, validEntities) {
    const errors = [];
    // 1. type 校验
 ...
 * 将校验错误转化为 PM 可读的追问消息
 */
export function validationErrorsToQuestions(errors) {
    return errors
        .filter(e => e.severity === 'error')
        .map(e => e.message);
}
//# sourceMappingURL=requirement-validator.js.map
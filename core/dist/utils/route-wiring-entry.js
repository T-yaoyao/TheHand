...
export function isRouteTableModulePath(relPath) {
    return ROUTE_TABLE_RE.test(relPath.replace(/\\/g, '/'));
}
function isEntryFileName(rel) {
 ...
 */
export function pickApplicationEntryForRouteTable(sandboxPath, projectContext) {
    const ordered = [];
    const th = projectContext?.thehand;
    for (const p of th?.routingIntegration?.routeEntryFiles ?? []) {
        const rel = p.replace(/\\/g, '/');
        if (rel && !isRouteTableModulePath(rel))
            ordered.push(rel);
    }
    for (const p of th?.recallL1?.entryCandidates ?? []) {
        const rel = p.replace(/\\/g, '/');
        if (rel && !isRouteTableModulePath(rel) && isEntryFileName(rel))
            ordered.push(rel);
    }
    const seen = new Set();
 ...
 */
export function collectRouteIntegrationContextPaths(sandboxPath, projectContext) {
    const out = [];
    const seen = new Set();
    const th = projectContext?.thehand;
    const push = (rel) => {
        if (out.length >= ROUTE_INJECT_MAX)
            return;
        if (!rel)
            return;
        ...
        if (!n || seen.has(n))
            return;
        if (isRouteTableModulePath(n))
            return;
        if (!existsSync(resolve(sandboxPath, n)))
            return;
        seen.add(n);
        out.push(n);
    };
    ...
    for (const c of th?.readContextCandidates ?? []) {
        if (rc >= 3 || out.length >= ROUTE_INJECT_MAX)
            break;
...
//# sourceMappingURL=route-wiring-entry.js.map
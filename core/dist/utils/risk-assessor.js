...
export class RiskAssessor {
    ...
    getEntityScore(entity) {
        const simpleEntities = ['Tag', 'ArticlePreview', 'Comment'];
        const complexEntities = ['User', 'Article'];
        if (simpleEntities.includes(entity))
            return 90;
        if (complexEntities.includes(entity))
            return 70;
        return 60;
    }
    generateRecommendations(riskLevel, plan) {
        const recs = [];
        if (riskLevel === 'low') {

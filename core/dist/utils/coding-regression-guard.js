 ...
 */
export function detectCodingRegression(original, generated) {
    const reasons = [];
    const o = original;
    const g = generated;
    if (!o.trim() || o === '（新文件，不存在）')
    ...
    const suspicious = hookLost ||
        reasons.length >= 2 ||
        (reasons.length >= 1 && oLen > 800 && gLen < oLen * 0.38);
    return { suspicious, reasons };
}
export function formatRegressionRetryHint(pathsAndReasons) {
    if (pathsAndReasons.length === 0)
        return '';
    const lines = pathsAndReasons.map(({ path, reasons }) => `- **${path}**：${reasons.join('；')}`);
    return `
...
//# sourceMappingURL=coding-regression-guard.js.map
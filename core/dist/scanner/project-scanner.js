 ...
 */
export async function scanProject(sandboxPath, llmClient) {
    const constraints = {};
    const contextFiles = [];
    const semanticTags = [];
    ...
    if (orm) {
        ...
        if (orm.modelFieldPattern)
            ormPatterns.modelField = orm.modelFieldPattern;
    ...
    if (routing) {
        ...
        if (routing.contextFile)
            contextFiles.push(routing.contextFile);
        if (routing.semanticTags)
            semanticTags.push(...routing.semanticTags);
    ...
    if (llmClient) {
        try {
            ...
            diagnostics.push(`llm: failed (${err.message?.slice(0, 50)})`);
        }
    }
    // ── 11. 通用修改规则 ──
 ...
 */
async function analyzeProjectWithLLM(sandboxPath, llmClient, existingConstraints, contextFiles) {
    ...
    const response = await llmClient.simpleChat(`You are a project convention analyzer. Given the following project files, output a JSON object of additional project conventions/constraints that a code generator should follow. Focus on patterns NOT already covered by these existing constraints:\n${existing}\n\nOutput ONLY a JSON object where keys are constraint names and values are short descriptions. No markdown, no explanation.`, fileContents.join('\n\n'), 'scanner');
    // 解析 LLM 返回的 JSON
    try {
 ...
 */
async function readSampleComponent(sandboxPath) {
...
async function detectRoutingPattern(sandboxPath, deps) {
    // React Router
...
async function detectComponentStructure(sandboxPath) {
...
function detectStylingPattern(deps, sandboxPath) {
    if (deps?.['styled-components'])
        return '样式使用 styled-components（CSS-in-JS）';
    if (deps?.['@emotion/react'] || deps?.['@emotion/styled'])
        return '样式使用 Emotion（CSS-in-JS）';
    if (deps?.['tailwindcss'])
        return '样式使用 Tailwind CSS';
...
function detectAPIPattern(deps) {
    if (deps?.['express'] && deps?.['sequelize'])
        return '后端 API 在 routes/ 中定义，使用 Express Router + Sequelize ORM';
    if (deps?.['express'])
        return '后端 API 在 routes/ 中定义，使用 Express Router';
    if (deps?.['koa'])
        return '后端 API 使用 Koa Router';
    if (deps?.['fastify'])
        return '后端 API 使用 Fastify 路由';
    return null;
}
function detectDependencyConstraints(deps) {
    if (!deps)
        return null;
...
async function detectEntryFiles(sandboxPath) {
    const candidates = [
...
async function findFile(base, candidates) {

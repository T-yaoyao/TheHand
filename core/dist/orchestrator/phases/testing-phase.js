...
export class TestingPhase extends BasePhaseHandler {
    ...
    async *execute(ctx) {
        ...
        // 从 CodingPhase 传递的数据
        const codeOutputs = ctx._codingOutputs ?? [];
        const fileValidationSummary = ctx._fileValidationSummary ?? null;
        const pastLessons = ctx._pastLessons ?? [];
    ...
    async *runBoundaryTests(ctx, codeOutputs, commands) {
        ...
        try {
            ...
            const boundaryTestResult = await agentRunner.run(createBoundaryTestAgent(), {
    ...
    async *generateDiffAndPause(ctx, codeOutputs, fileValidationSummary) {
...
//# sourceMappingURL=testing-phase.js.map
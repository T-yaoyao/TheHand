...
/** 暂停点集合（保留沙箱，等待用户操作） */
export const PAUSE_POINTS = new Set([
    'waiting-for-pm',
    'needs-confirmation',
    'plan-ready',
    'diff-ready',
 ...
 * @throws Error 非法转换
 */
export function transition(requirement, to, reason) {
    const from = requirement.status;
    if (!canTransition(from, to)) {
        throw new StateTransitionError(`非法状态转换: ${from} → ${to}` + (reason ? ` (${reason})` : ''), from, to);
    }
 ...
 * 检查是否为暂停点（需要保留沙箱）
 */
export function isPausePoint(status) {
    return PAUSE_POINTS.has(status);
}
/**
 * 获取从当前状态可以到达的所有合法状态
 */
export function getValidNextStates(status) {

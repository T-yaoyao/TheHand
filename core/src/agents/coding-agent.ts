...
export async function runCoding(
  ...
  lessonsHint?: string,
  previousOutputs?: CodeFileOutput[],
  testError?: string,
...
export async function runCodingBatch(
  ...
  architectFiles?: ArchitectFileAnalysis[],
  ...
  if (crossFileRefs && crossFileRefs.length > 0) {
    ...
    const relevantRefs = crossFileRefs.filter(r =>
      batchFileSetForRef.has(r.from) || batchFileSetForRef.has(r.to)
    )
 ...
 */
function parseCodingBatchResponse(response: string, plan: FilePlan[]): CodeFileOutput[] {
  if (!response?.trim()) return []

  const results: CodeFileOutput[] = []

  // 辅助：用深度计数找最外层 JSON 片段，提取所有能解析的文件，绝不提前退出
  function tryExtractByBracket(open: string, close: string): void {
    let start = -1
    let depth = 0
    let inString = false
    let escape = false
...
function extractFiles(parsed: any, results: CodeFileOutput[]): void {
  const items: any[] = []
  if (Array.isArray(parsed)) items.push(...parsed)
  else if (parsed?.files && Array.isArray(parsed.files)) items.push(...parsed.files)
  else if (parsed?.path && parsed?.content) items.push(parsed)
 ...
 * 获取项目目录树（限制深度）
 */
async function getProjectTree(dirPath: string, maxDepth: number, currentDepth = 0, prefix = ''): Promise<string> {
  ...
  try {
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      if (entry.isDirectory()) {
        const subTree = await getProjectTree(
 ...
 */
export async function extractAvailableDependencies(sandboxPath: string): Promise<Set<string>> {
...
export function validateImports(
  outputs: CodeFileOutput[],
  availableDeps: Set<string>,
): { file: string; imp: string }[] {
  const violations: { file: string; imp: string }[] = []
  // 匹配 import ... from 'xxx' 和 require('xxx')
  const importRegex = /(?:import\s+.*?from\s+['"]([^'"]+)['"]|require\s*\(\s*['"]([^'"]+)['"]\s*\))/g

  for (const file of outputs) {
    if (file.content === '__DELETE__') continue
    if (!/\.(jsx?|tsx?|mjs|cjs|vue)$/.test(file.path)) continue
...
export function parseCodingFileResponse(
  ...
  if (codeMatch) {
    ...
    if (fromBlockJson) return fromBlockJson
...
function tryParseCodingJson(
  ...
  else if (Array.isArray(json.files)) items.push(...json.files)

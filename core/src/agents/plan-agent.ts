    ...
    parameters: {
      ...
      properties: {
        ...
        includeRouteEntryContext: {
          type: 'boolean',
          description:
            '是否需要在编码阶段自动并入应用路由入口（如 main.jsx）及 project.json 的 readContextCandidates。凡前端方案含 routes/ 下 jsx|tsx 且需求涉及 Tab/子路由/嵌套/Outlet/NavItem/NavLink/新 path 须为 true，以便首轮改路由；仅同一路由下纯 UI/数据展示、不动 path 与 Tab 为 false。',
        },
      },

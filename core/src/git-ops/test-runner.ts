 ...
 */
function extractMissingPackages(output: string): string[] {
...
export class TestRunner {
    ...
    while (!testResult.passed) {
      ...
      if (missingPkgs.length > 0 && fixAttempts === 1) {
        ...
        const installResult = await this.executeStep(
          `install-missing-deps (${missingPkgs.join(', ')})`,
          `npm install ${missingPkgs.join(' ')} --save-dev`,
          180_000, // npm install 可能较慢
        )
   ...
   * 执行单个测试步骤
   */
  private async executeStep(name: string, command: string, timeout: number = 120000): Promise<TestStepResult> {

...
export class DockerSandboxManager {
  ...
  async create(id?: string): Promise<Sandbox> {
    ...
    try {
      ...
      // 创建失败时清理资源
      if (containerStarted) {
        try { await execAsync(`docker rm -f ${containerName}`, { timeout: 10_000 }) } catch {}
      }
  ...
  async applyToSource(sandbox: Sandbox, files: string[], commitMessage?: string): Promise<void> {
    ...
    for (const file of files) {
      ...
      // 路径穿越检查
      if (!src.startsWith(resolve(sandbox.path)) || !dest.startsWith(resolvedSource)) continue
    ...
    while (Date.now() < deadline) {
      try {
        const result = await this.dockerExec(
          entry.containerName,
          `node -e "const http=require('http');const req=http.get('http://localhost:${port}',r=>{console.log(r.statusCode);process.exit(0)});req.on('error',()=>process.exit(1));req.setTimeout(2000,()=>process.exit(1))"`,
          5_000,
        )

 ...
 */
export function getTheHandRoot(): string {
...
export function resolveProjectsDir(): string {
  ...
  if (!raw) {
    throw new Error(
      '[thehand] 缺少必须环境变量 THEHAND_PROJECTS_DIR（示例：projects）。请见仓库根目录 .env.example',
    )
  }
 ...
 */
export function resolveSandboxRepoAbs(): string {
  const root = getTheHandRoot()
  const raw = process.env.THEHAND_SANDBOX_REPO?.trim()
  if (!raw) {
    throw new Error(
      '[thehand] 缺少必须环境变量 THEHAND_SANDBOX_REPO（示例：sandbox-repo/conduit-realworld-example-app）。请见仓库根目录 .env.example',
    )
  }
  return isAbsolute(raw) ? raw : resolve(root, raw)
}
...
export function getDefaultProjectId(): string {
  ...
  if (!id) {
    throw new Error(
      '[thehand] 缺少必须环境变量 THEHAND_DEFAULT_PROJECT_ID（示例：conduit）。请见仓库根目录 .env.example',
    )
  }
  return id
}
...
export function assertTheHandRequiredEnv(): void {
  getDefaultProjectId()

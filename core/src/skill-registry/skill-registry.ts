...
import type { SkillDefinition, StructuredRequirement, ProjectContext, SkillOutput, SkillMatchRule } from '../types.js'
...
export class SkillRegistry {
  ...
  private matchRules: Map<string, SkillMatchRule> = new Map()
  ...
  match(requirement: StructuredRequirement): SkillDefinition | null {
    ...
    const declarativeMatches: { name: string; score: number; priority: number }[] = []
   ...
   * 返回 0 表示不匹配，>0 表示匹配度
   */
  private evaluateMatchRule(rule: SkillMatchRule, req: StructuredRequirement): number {
    let score = 0

    // type 匹配
    if (rule.type) {
      const types = Array.isArray(rule.type) ? rule.type : [rule.type]
      if (types.includes(req.type)) score += 10
      else return 0  // type 不匹配直接排除
    }

    // entity 匹配
    if (rule.entity) {
      const entities = Array.isArray(rule.entity) ? rule.entity : [rule.entity]
      if (entities.includes(req.entity)) score += 5
      else return 0
    }
    ...
    if (rule.scope) {
      ...
      if (scopes.includes(req.scope)) score += 3
      else return 0
    }
...
3. 只修改必要的内容`
   ...
   */
  private tryParseAsMatchRule(expression: string): SkillMatchRule | null {
    ...
    if (scopeMatch) {
      const scopes = scopeMatch[1].split(',').map(s => s.trim()) as ('frontend' | 'backend' | 'fullstack')[]
      rule.scope = scopes
      hasAny = true
    }

...
import {
  ...
  loadSandboxSourceContents,
  relativeSpecifierResolvesInSandbox,
} from './component-sandbox-import.js'
...
function extractRelativeImportsLoose(content: string): string[] {
  const out = new Set<string>()
  for (const s of extractRelativeImportSpecifiers(content)) out.add(s)
 ...
 */
export async function findUnresolvedRelativeImportsInFileMap(
  sandboxPath: string,
  ...
  for (const [k, v] of fileMap) {
    if (!v.content || v.content === '__DELETE__') continue
    baseIndex.set(k.replace(/\\/g, '/'), v.content)
  }
  const known = new Set(baseIndex.keys())

  const violations: UnresolvedRelativeImportViolation[] = []
  ...
  for (const relPath of pathsToScan) {
    ...
    for (const spec of extractRelativeImportsLoose(content)) {
      ...
      violations.push({
        ...
        specifier: spec,
        message:
          `文件 ${from} 中相对 import「${spec}」无法解析到沙箱内任何现有文件（含本轮已生成路径）。` +

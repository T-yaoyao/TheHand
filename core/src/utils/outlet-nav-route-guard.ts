...
export function extractOutletRelativeNavSegments(content: string): string[] {
  if (!/<Outlet\b/.test(content)) return []

  const found = new Set<string>()

  const collectFromTags = (tagRe: RegExp, prop: 'url' | 'to') => {
    tagRe.lastIndex = 0
...
export function routeTableDeclaresSegment(routeSources: string, segment: string): boolean {
...
function collectRouteSourcePaths(
  ...
  for (const p of projectContext?.thehand?.routingIntegration?.routeEntryFiles ?? []) push(p)
  for (const p of projectContext?.thehand?.recallL1?.entryCandidates ?? []) {
    const base = p.split('/').pop() ?? ''
    if (/^(main|index|App)\.(jsx|tsx|js|ts)$/i.test(base)) push(p)
  }
  for (const f of planFiles) {
    if (isRouteTableModulePath(f.path)) push(f.path)
  }
  for (const p of fileMapPaths) {
    if (isRouteTableModulePath(p)) push(p)
  }
  for (const p of ['frontend/src/main.jsx', 'frontend/src/main.tsx', 'src/main.jsx', 'src/main.tsx']) push(p)
 ...
 */
export async function loadMergedRouteTableSources(
  sandboxPath: string,
  projectContext: ProjectContext | undefined,
  planFiles: FilePlan[],
...
async function loadSourcesJoined(
  sandboxPath: string,
  paths: string[],
  fileMap: Map<string, { path: string; content: string }>,
): Promise<string> {
...
async function readPlanOrMapContent(
  relPath: string,
  fileMap: Map<string, { path: string; content: string }>,
  sandboxPath: string,
): Promise<string | null> {
  const mapped = fileMap.get(relPath)?.content
  if (mapped && mapped !== '__DELETE__') return mapped
  try {
 ...
 */
export async function findOutletNavRouteViolations(
  sandboxPath: string,
  projectContext: ProjectContext | undefined,
  planFiles: FilePlan[],
  fileMap: Map<string, { path: string; content: string }>,
  ...
  if (!routeSources.trim()) return []

  const violations: OutletNavRouteViolation[] = []
  const seen = new Set<string>()

  const pathsToScan = new Set<string>()
  for (const f of planFiles) {
    if (f.path) pathsToScan.add(f.path.replace(/\\/g, '/'))
  }
  for (const k of fileMap.keys()) pathsToScan.add(k.replace(/\\/g, '/'))

  for (const relPath of pathsToScan) {
    if (!/\.(jsx|tsx)$/.test(relPath)) continue
    ...
    for (const seg of segments) {
      if (routeTableDeclaresSegment(routeSources, seg)) continue
      ...
      violations.push({
        ...
        segment: seg,
        message:
          `布局 ${relPath} 在含 <Outlet> 的页面中为相对路径导航声明了「${seg}」，但路由入口（如 main.jsx / router）中未找到 path="${seg}" 的 <Route>，运行时将 404。请在路由表中为该段增加嵌套 <Route path="${seg}" element={...} />，与 NavItem/NavLink 一致。`,
      })
    }
  }

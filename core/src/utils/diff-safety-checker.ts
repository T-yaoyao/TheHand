...
export class DiffSafetyChecker {
  ...
  async check(originalFiles: Map<string, string>, plan: FilePlan[]): Promise<DiffCheckResult> {
    const unexpectedFiles: string[] = []
    const unrelatedChanges: DiffCheckResult['unrelatedChanges'] = []
  ...
  private findUnrelatedChanges(original: string, modified: string): number[] {
    const originalLines = original.split('\n')
    const modifiedLines = modified.split('\n')
    const unrelatedLines: number[] = []
  ...
  private isTrivialChange(orig: string, mod: string): boolean {
    // 空行变化

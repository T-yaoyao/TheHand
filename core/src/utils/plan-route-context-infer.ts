...
const NESTED_ROUTE_TAB_SIGNAL =
  /Tab|标签页|子路由|嵌套路由|嵌套\s*path|Outlet|NavItem|NavLink|同级路由|新\s*path|path\s*=\s*["']|path:\s*["']/i
 ...
 */
export function inferRouteEntryContextFromRequirementAndPlan(
  structured: StructuredRequirement | null | undefined,
  plan: FilePlan[],
  pmInput: string | undefined,
): boolean {
  if (!structured) return false
  const scope = structured.scope ?? ''
  if (!/^(frontend|fullstack)$/i.test(scope.trim())) return false

  const text = [structured.description ?? '', pmInput ?? ''].join('\n')
  if (!NESTED_ROUTE_TAB_SIGNAL.test(text)) return false

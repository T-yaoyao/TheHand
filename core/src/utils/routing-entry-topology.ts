...
export function entryFileDeclaresRoutesJsx(entrySource: string): boolean {
 ...
 */
export function entryImportsRelativeRouterModule(entrySource: string): boolean {
  const sideEffect =
    /\bimport\s+['"]\.\/router(?:\.(?:jsx?|tsx?|js|ts))?['"]/i.test(entrySource) ||
    /\bimport\s+['"]\.\.\/router(?:\.(?:jsx?|tsx?|js|ts))?['"]/i.test(entrySource)
  const namedOrDefault =
    /\bfrom\s+['"]\.\/router(?:\.(?:jsx?|tsx?|js|ts))?['"]/i.test(entrySource) ||
    /\bfrom\s+['"]\.\.\/router(?:\.(?:jsx?|tsx?|js|ts))?['"]/i.test(entrySource)
  return sideEffect || namedOrDefault
}

function readEntrySource(sandboxPath: string, entryRel: string | null): string | null {
 ...
 */
export function isPhantomRouteTableVersusEntryTopology(
  planRelPath: string,
  sandboxPath: string,
  ...
  if (!isRouteTableModulePath(p)) return false
  if (existsSync(resolve(sandboxPath, p))) return false
  if (!entryRel || !entrySource) return false
  if (!entryFileDeclaresRoutesJsx(entrySource)) return false
  if (entryImportsRelativeRouterModule(entrySource)) return false
  return true
}
 ...
 */
export function sanitizePhantomRouteTablesAgainstEntryTopology(
  plan: FilePlan[],
  sandboxPath: string,
  ...
  if (!entry || !entrySource) return plan

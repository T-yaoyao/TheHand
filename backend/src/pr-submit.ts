...
function gh(cwd: string, args: string[], timeout = 180_000): string {
...
export function pushBranchAndCreatePrToMain(options: {
  ...
  let createdLocalBranch: string | null = null
  ...
  try {
    prUrl = gh(
      cwd,
      ['pr', 'create', '--base', base, '--head', pushBranch, '--title', title, '--body-file', bodyFile],
      180_000,
    )
  } finally {
    try {
